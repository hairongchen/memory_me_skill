---
name: memory-me
description: 在~/.claude/目录下初始化记忆系统，创建memory_me目录，并在改目录下生成 CLAUDE.md（可选 AGENT.md 给 Cursor 用）、MEMORY.md 和 memory/ 目录。当用户说"初始化记忆"、"搭建记忆"、"memory me"、"/memory-me"时触发。
---
...
在memory_me目录下一键部署记忆系统，生成三个东西：

1. **CLAUDE.md** — 人设 + 对话规则 + 记忆协议
2. **MEMORY.md** — 长期记忆文件（初始为空模板）
3. **memory/** — 每日记忆目录

- `/memory-me` → 交互式初始化，逐步收集信息后生成
...
先检查memory_me目录下是否已存在 CLAUDE.md、AGENT.md、MEMORY.md 或 memory/ 目录。
...
4. **有什么特别的工作习惯要我知道的？比如主要工作语言、沟通习惯等**（可选，用户可以跳过）
...
## 工作流
1. 对话开始 → 用 Read 工具读取memory_me目录下的 MEMORY.md，再用 Read 读取 memory/当天日期.md（如 memory/2026-03-02.md），了解今天已有的上下文
2. 涉及历史上下文 → 用 Grep 工具搜索memory_me目录下的 memory/ 目录
3. 对话结束 → 问是否要记录，确认后写入 memory/当天日期.md
4. 如果产出了值得长期记住的信息（如新客户、重要决策），同步更新 MEMORY.md
5. 注意：以上所有文件路径都是相对于memory_me目录

## 补充规则
- 记忆以memory_me目录下的文件为准，忽略 ~/.claude/ 下的 auto-memory（内置记忆系统），不要读也不要写
